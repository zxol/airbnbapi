...
class AirApi {
    ...
    async login({email, password} = {}) {
        if(!email) {
            logger.error('Airbnbapi: Can\'t login without an email.')
            logger.error('email: ' + email)
            logger.error('Please call login({email: \'#######\', password: \'#######\'})')
            return null
        }
        else if(!password) {
            logger.error('Airbnbapi: Can\'t apply for a token without a password.')
        ...
        try {
            ...
            // }
            // console.log(JSON.stringify(e, null, 4))
            logger.error('Airbnbapi: Couldn\'t get auth token for ' + email )
            logger.error(e.error)
            return { error: e.error }
        }
        if(response && response.login) {
            logger.info('Airbnbapi: Successful login for [ ' + email + ' ], auth ID is [ ' + response.login + ' ]')
    ...
    async setPriceForDay({token, id, date, price, currency = this.config.DEFAULT_PARAMETERS.currency}) {
        const options = {
            method: 'PUT',
        ...
        try {
            ...
            logger.error('Airbnbapi: Couldn\'t set price for cal day for listing ' + id )
            logger.error(e)
        }
    }
    ...
    async setAvailabilityForDay({token, id, date, availability}) {
        ...
        try {
            ...
            logger.error('Airbnbapi: Couldn\'t set availability for cal day for listing ' + id )
            logger.error(e)
        }
    }
    ...
    async sendReview({
        ...
        if(!token) {
            logger.error('Airbnbapi: Can\'t send a review without a token')
            return null
        } else if(!id) {
            logger.error('Airbnbapi: Can\'t send review without an id')
            return null
        }
        const options = {
            method: 'POST',
        ...
        try {
            ...
            logger.error('Airbnbapi: Couldn\'t send a review for thread  ' + thread_id )
            logger.error(e)
        } finally {
            return response
        }
    ...
    async sendSpecialOffer({
        ...
        if(!token) {
            logger.error('Airbnbapi: Can\'t send a special offer without a token')
            return null
        } else if(!startDate) {
            logger.error('Airbnbapi: Can\'t send a special offer without a startDate')
            return null
        } else if(!guests) {
            logger.error('Airbnbapi: Can\'t send a special offer without guests')
            return null
        } else if(!listingId) {
            logger.error('Airbnbapi: Can\'t send a special offer without a listingId')
            return null
        } else if(!nights) {
            logger.error('Airbnbapi: Can\'t send a special offer without nights (staying)')
            return null
        } else if(!price) {
            logger.error('Airbnbapi: Can\'t send a special offer without a price')
            return null
        }  else if(!threadId) {
            logger.error('Airbnbapi: Can\'t send a special offer without a threadId')
            return null
        }
    ...
    {
        if(!token) {
            logger.error('Airbnbapi: Can\'t get user info without a token')
            return null
        }
        const options = {
            method: 'GET',
        ...
        try {
            ...
            logger.error('Airbnbapi: Couldn\'t get own info with token' + token )
            logger.error(e)
        } finally {
            return (response && response.user) ? response.user : null
        }
