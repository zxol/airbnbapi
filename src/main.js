...
class AirApi {
    ...
    async setBasePrice({ token, listing_id, daily_price, currency = this.config.currency }) {
        ...
        try {
            ...
            log.e("Airbnbapi: Couldn't set listing price for listing " + listing_id)
            log.e(e)
        }
    }
    ...
    async getListingsPerformance({
        ...
        if (!(token || this.config.token)) {
            log.e("Airbnbapi: Can't get listings performance without a token")
            return null
        }
        if (!startDate || typeof startDate != 'string') {
            log.e("Airbnbapi: Can't get a performance data without a start date ( ISOString )")
            return null
        } else if (!endDate || typeof endDate != 'string') {
            log.e("Airbnbapi: Can't get a performance data without an end date ( ISOString )")
            return null
        }
        const options = this.buildOptions({
            method: 'POST',
        ...
        try {
            ...
            log.e("Airbnbapi: Couldn't get a listings performance for token " + token)
            log.e(e)
        }
    }
